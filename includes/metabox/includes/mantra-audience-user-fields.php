<?php
if( ! class_exists( 'Mantra_Audience_User_Meta' ) ) :
/**
...
class Mantra_Audience_User_Meta {
	 ...
	 *
	 * @uses apply_filters calls mantra_audience_metabox/user/fields filter
	 * @return array
	 */
	public function get_fields() {
		if( $this->fields == null ) {
			$this->fields = apply_filters( 'mantra_audience_metabox/user/fields', array() );
		}
	...
	function user_fields( $user ) {
		...
		foreach( $groups as $id => $group ) : ?>
			...
			<?php
			$post_id = 0;
			$mantra_audience_custom_panel_nonce = wp_create_nonce( "mantra-audience-custom-panel" );
			foreach( $group['fields'] as $field ) :
				...
				?>
				<tr class="mantra_audience_field_row">
					<th><label for=""><?php echo $field['title']; ?></label></th>
					<td class="mantra_audience_field">
						<?php
						do_action( "mantra_audience_metabox/field/{$field['type']}", array(
							'meta_box' => $field,
	...
	function enqueue() {
		...
		if ( in_array( $pagenow, array( 'profile.php', 'user-edit.php' ) ) ) {
			Mantra_Audience_Metabox::get_instance()->enqueue();
		}
	}
}
endif;
Mantra_Audience_User_Meta::get_instance();