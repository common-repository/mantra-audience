...
foreach( array( 'image', 'audio', 'postmeta', 'post_id_info', 'multi', 'date', 'video', 'color', 'dropdown', 'dropdownbutton', 'textbox', 'textarea', 'checkbox', 'separator', 'layout', 'radio', 'gallery_shortcode', 'query_category', 'assignments', 'repeater', 'image_radio' ) as $field_type ) {
	add_action( "mantra_audience_metabox/field/{$field_type}", "mantra_audience_meta_field_{$field_type}", 10, 1 );
}
add_action( 'wp_ajax_mantra_audience_metabox_media_lib_browse', 'mantra_audience_metabox_media_lib_browse' );
add_action( 'wp_ajax_mantra_audience_plupload', 'mantra_audience_wp_ajax_plupload_image' );

function mantra_audience_meta_field_image( $args ) {
	...
	// backward compatibility with mantraAudience framework, image fields that are used to replace the Featured Image
	$mantra_audience_featimg_field = function_exists( 'mantra_audience_is_thumbnail_field' ) && mantra_audience_is_thumbnail_field( $meta_box['name'] ) ? true : false;
	if( $mantra_audience_featimg_field ) {
		$featimg_uploader_args['featured'] = $meta_box['name'];
	}

	$this_attachment_id = '';
	if( has_post_thumbnail() && $mantra_audience_featimg_field ) {
		$this_attachment_id = get_post_thumbnail_id( $post_id );
		$thumb = wp_get_attachment_image_src( $this_attachment_id );
		$thumb_src = $thumb[0];
	} elseif( !$mantra_audience_featimg_field && ( $this_attachment_id = get_post_meta( $post_id, '_'.$meta_box['name'].'_attach_id', true ) ) ) {
		$thumb = wp_get_attachment_image_src( $this_attachment_id );
		$thumb_src = $thumb[0];
	} else {
	...
	<div id="<?php echo esc_attr( 'remove-' . $meta_box['name'] ); ?>" class="mantra_audience_featimg_remove <?php if( $thumb_src == '' ) echo 'hide' ?>">
		<a data-attachid="<?php echo esc_attr( get_post_meta( $post_id, '_' . $meta_box['name'] . '_attach_id', true ) ); ?>" href="#"><?php _e( 'Remove image', 'mantra-audience' ); ?></a>
	</div>
	...
	if( $thumb_src ) {
		...
		echo '<div class="mantra_audience_upload_preview" style="display:block;"><a href="' . esc_url( $full_src ) . '" target="_blank"><img src="' . esc_url( $thumb_src ) . '" width="40" alt="' . esc_attr__( 'Post Image', 'mantra-audience' ) . '" /></a></div>';
	} else {
		echo '<div class="mantra_audience_upload_preview"></div>';
	}
	?>

	<!-- Field storing URL -->
	<input type="hidden" id="<?php echo esc_attr( $meta_box['name'] ); ?>" name="<?php echo esc_attr( $meta_box['name'] ); ?>" value="<?php echo esc_attr( $meta_value ); ?>" size="55" class="mantra_audience_input_field mantra_audience_upload_field" />

	<div class="mantra_audience_upload_buttons">
		<?php mantra_audience_uploader($meta_box['name'], $featimg_uploader_args) ?>
	</div>

	<?php if ( isset( $meta_box['description'] ) ) : ?>
		<span class="mantra_audience_field_description"><?php echo wp_kses_post( $meta_box['description'] ); ?></span>
	...
	<script type="text/javascript" defer>
		jQuery(function($){
			...
			$remove.find('a').on('click', function(e){
				...
				$.post(
					ajaxurl, {
						'action': 'mantra_audience_remove_post_image',
						'postid': <?php echo esc_js( $post_id ); ?>,
						<?php if( !$mantra_audience_featimg_field )
								echo '\'attach_id\': $this.data(\'attachid\'),'; ?>
						'customfield' : '<?php echo esc_js( $meta_box['name'] ); ?>',
						'nonce' : '<?php echo esc_js( $mantra_audience_custom_panel_nonce ); ?>'
					},
					function() {
						$this.parent().parent().find('.mantra_audience_upload_field').val('');
						$this.parent().parent().find('.mantra_audience_upload_preview').fadeOut();
...
function mantra_audience_meta_field_audio( $args ) {
	...
	$remove_data = array(
		...
		'nonce'			=> esc_js( $mantra_audience_custom_panel_nonce )
	); ?>

	<div
		id="<?php echo esc_attr( 'remove-' . $sanitized_name ); ?>"
		class="mantra_audience_featimg_remove mantra_audience_video_remove <?php $meta_value == '' && print( 'hide' ); ?>"
		data-audio-remove='<?php echo json_encode( $remove_data ); ?>'>
			<a href="#"><?php _e( 'Remove Audio', 'mantra-audience' ); ?></a>
	</div>
	...
	<input
		...
		class="mantra_audience_input_field mantra_audience_upload_field"
		name="<?php echo esc_attr( $meta_box['name'] ); ?>">

	<div class="mantra_audience_upload_buttons">
		<?php mantra_audience_uploader( $sanitized_name, $featimg_uploader_args) ?>
	</div>

	<?php if ( isset( $meta_box['description'] ) ) : ?>
		<span class="mantra_audience_field_description">
...
function mantra_audience_meta_field_postmeta( $args ) {
	extract( $args, EXTR_OVERWRITE );
...
function mantra_audience_meta_field_post_id_info( $args ) {
	extract( $args, EXTR_OVERWRITE );
	?>

	<span class="mantra_audience_field_description mantra_audience_field_info"><?php echo sprintf( $meta_box['description'], $post_id ); ?></span>
	<?php
}

/**
 * Generates date picker field for mantraAudience Custom Panel.
 *
...
function mantra_audience_meta_field_date( $args ) {
	...
	$pick  = isset( $meta_box['meta']['pick'] ) && '' != $meta_box['meta']['pick'] ? $meta_box['meta']['pick']  : __( 'Pick Date', 'mantra-audience' );
	$close = isset( $meta_box['meta']['close'] ) && '' != $meta_box['meta']['close'] ? $meta_box['meta']['close'] : __( 'Done', 'mantra-audience' );
	$clear = isset( $meta_box['meta']['clear'] ) && '' != $meta_box['meta']['clear'] ? $meta_box['meta']['clear'] : __( 'Clear', 'mantra-audience' );
	...
	$html = sprintf( '
		...
		<input %s type="text" id="%s" name="%s" value="%s" data-clear="%s" data-label="%s"
		 data-close="%s"
		 data-dateformat="%s" data-timeformat="%s" data-timeseparator="%s" class="mantra_audience_input_field medium mantraAudienceDatePicker" data-first-day="%s" />
		<input type="button" id="%s" data-picker="%s" value="%s" class="button mantraAudienceClearDate mantraAudienceOpacityTransition %s" >',
	...
	'clear-'.esc_attr( $meta_box['name'] ), esc_attr( $meta_box['name'] ), esc_attr( $clear ), $meta_value? 'mantraAudienceFadeIn' : '' );
	...
	$html .= isset( $meta_box['description'] )? mantra_audience_meta_field_get_description($meta_box['description']) : '';
...
function mantra_audience_meta_field_color( $args ) {
	...
	$html = sprintf( '
	<input type="text" id="%s" name="%s" value="%s" class="mantra_audience_input_field colorSelectInput" data-format="%s" />
	<input type="button" class="button clearColor" value="' . __('&times;', 'mantra-audience') . '">',
 ...
 * @return string
 */
function mantra_audience_meta_field_dropdownbutton( $args ) {
	$meta_box = $args['meta_box'];
	$meta_value = $args['meta_value'];
	$name = '';
	extract($args, EXTR_OVERWRITE);
	...
	foreach($meta_box['states'] as $state) {
		$state['icon'] = sprintf( $state['icon'], MANTRA_AUDIENCE_METABOX_URI . '/img' );
	...
	$html = mantra_audience_meta_field_get_label($html, $meta_box);
 ...
 * @return string
 */
function mantra_audience_meta_field_radio( $args ) {
	$meta_box = $args['meta_box'];
	$meta_value = $args['meta_value'];
	extract($args, EXTR_OVERWRITE);
...
function mantra_audience_meta_field_separator( $args ) {
	...
	$out = '<div class="mantra_audience_field_row clearfix ' . esc_attr( $toggle_class ) . '" ' . esc_attr( $ext_attr );
...
function mantra_audience_meta_field_checkbox( $args ) {
	...
	if ( isset( $meta_box['description'] ) ) {
		$html .= mantra_audience_meta_field_get_description( $meta_box['description'] );
	}
...
function mantra_audience_meta_field_textbox( $args ) {
	...
	$html = sprintf('<input type="text" id="%s" name="%s" value="%s" size="55" class="mantra_audience_input_field %s" />',
...
function mantra_audience_meta_field_textarea( $args ) {
	...
	$html = sprintf( '<textarea id="%s" name="%s" size="%s" rows="%s" class="mantra_audience_input_field %s">%s</textarea>',
...
function mantra_audience_meta_field_video( $args ) {
	global $post;
	...
	<div id="<?php echo esc_attr( 'remove-' . $meta_box['name'] ); ?>" class="mantra_audience_featimg_remove mantra_audience_video_remove <?php if( $meta_value == '' ) echo 'hide' ?>">
		<a href="#"><?php _e('Remove Video', 'mantra-audience'); ?></a>
	</div>

	<!-- Field storing URL -->
	<input type="text" id="<?php echo esc_attr( $meta_box['name'] ); ?>" name="<?php echo $meta_box['name']; ?>" value="<?php echo esc_attr( $meta_value ); ?>" size="55" class="mantra_audience_input_field mantra_audience_upload_field" />
	...
	<script type="text/javascript" defer>
		jQuery(function($){
			$('#remove-<?php echo esc_js( $meta_box['name'] ); ?>').find('a').on('click', function(e){
				...
				$self.parent().find('.mantra_audience_upload_field').val('');
				$self.addClass('hide');

				$.post(
					ajaxurl, {
						'action': 'mantra_audience_remove_video',
						'postid': <?php echo esc_js( $post_id ); ?>,
						'customfield' : '<?php echo esc_js( $meta_box['name'] ); ?>',
						'nonce' : '<?php echo esc_js( $args['mantra_audience_custom_panel_nonce'] ); ?>'
					},
					function() {
						$self.parent().find('.mantra_audience_upload_field').val('');
						$self.addClass('hide');
	...
	$html = $html . '<!-- mantraAudience Video END -->';
	echo $html;
}
 ...
 * @return string
 */
function mantra_audience_meta_field_get_description( $desc = '' ) {
	return ( isset( $desc ) && '' != $desc ) ? '<span class="mantra_audience_field_description">' . wp_kses_post( $desc ) . '</span>' : '';
}

function mantra_audience_meta_field_assignments( $args ) {
	extract( $args );
	$field = $meta_box;
	$pre = $field['name'];
	$selected = $meta_value;
	if( '' == $selected )
		$selected = array();
	$post_types = apply_filters( 'mantra_audience_assignments_post_types', get_post_types( array( 'public' => true ) ) );
	...
	$taxonomies = apply_filters( 'mantra_audience_assignments_taxonomies', get_taxonomies( array( 'public' => true ) ) );
	...
	$output = '<div id="mantra_audience_assignments_'. $pre .'" class="mantra-audience-assignments"><ul class="clearfix">';

	/* build the tab links */
	$output .= '<li><a href="#' . $pre . '-assignment-tab-general">' . __( 'General', 'mantra-audience' ) . '</a></li>';
	$output .= '<li><a href="#' . $pre . '-assignment-tab-pages">' . __( 'Pages', 'mantra-audience' ) . '</a></li>';
	$output .= '<li><a href="#' . $pre . '-assignment-tab-categories-singles">' . __( 'Category Singles', 'mantra-audience' ) . '</a></li>';
	$output .= '<li><a href="#' . $pre . '-assignment-tab-categories">' . __( 'Categories', 'mantra-audience' ) . '</a></li>';
	$output .= '<li><a href="#' . $pre . '-assignment-tab-post-types">' . __( 'Post Types', 'mantra-audience' ) . '</a></li>';
	$output .= '<li><a href="#' . $pre . '-assignment-tab-taxonomies">' . __( 'Taxonomies', 'mantra-audience' ) . '</a></li>';
	$output .= '<li><a href="#' . $pre . '-assignment-tab-userroles">' . __( 'User Roles', 'mantra-audience' ) . '</a></li>';
	$output .= '</ul>';

	/* build the tab items */
	$output .= '<div id="' . $pre . '-assignment-tab-general" class="mantra-audience-assignment-options clearfix">';
	$checked = isset($selected['general']['home']) ? checked($selected['general']['home'], 'on', false) : '';
	$output .= '<label><input type="checkbox" name="' . $pre . '[general][home]" ' . $checked . ' />' . __( 'Home page', 'mantra-audience' ) . '</label>';
	$checked = isset($selected['general']['page']) ? checked($selected['general']['page'], 'on', false) : '';
	$output .= '<label><input type="checkbox" name="' . $pre . '[general][page]" ' . $checked . ' />' . __( 'Page views', 'mantra-audience' ) . '</label>';
	$checked = isset($selected['general']['single']) ? checked($selected['general']['single'], 'on', false) : '';
	$output .= '<label><input type="checkbox" name="' . $pre . '[general][single]" ' . $checked . ' />' . __( 'Single post views', 'mantra-audience' ) . '</label>';
	$checked = isset($selected['general']['search']) ? checked($selected['general']['search'], 'on', false) : '';
	$output .= '<label><input type="checkbox" name="' . $pre . '[general][search]" ' . $checked . ' />' . __( 'Search pages', 'mantra-audience' ) . '</label>';
	$checked = isset($selected['general']['category']) ? checked($selected['general']['category'], 'on', false) : '';
	$output .= '<label><input type="checkbox" name="' . $pre . '[general][category]" ' . $checked . ' />' . __( 'Category archive', 'mantra-audience' ) . '</label>';
	$checked = isset($selected['general']['tag']) ? checked($selected['general']['tag'], 'on', false) : '';
	$output .= '<label><input type="checkbox" name="' . $pre . '[general][tag]" ' . $checked . ' />' . __( 'Tag archive', 'mantra-audience' ) . '</label>';
	$checked = isset($selected['general']['author']) ? checked($selected['general']['author'], 'on', false) : '';
	$output .= '<label><input type="checkbox" name="' . $pre . '[general][author]" ' . $checked . ' />' . __( 'Author pages', 'mantra-audience' ) . '</label>';
	...
	foreach ( get_post_types( array( 'public' => true, 'exclude_from_search' => false, '_builtin' => false ) ) as $key => $post_type ) {
		...
		$output .= '<label><input type="checkbox" name="' . esc_attr( $pre . '[general][' . $key . ']' ) . '" ' . $checked . ' />' . sprintf( __( 'Single %s View', 'mantra-audience' ), $post_type->labels->singular_name ) . '</label>';
	}
	...
	foreach ( get_taxonomies( array( 'public' => true, '_builtin' => false ) ) as $key => $tax ) {
		...
		$checked = isset( $selected['general'][$key] ) ? checked( $selected['general'][$key], 'on', false ) : '';
		$output .= '<label><input type="checkbox" name="' . esc_attr( $pre . '[general][' . $key . ']' ) . '" ' . $checked . ' />' . sprintf( __( '%s Archive View', 'mantra-audience' ), $tax->labels->singular_name ) . '</label>';
	}

	$output .= '</div>'; // tab-general
	// Pages tab
	$output .= '<div id="' . $pre . '-assignment-tab-pages" class="mantra-audience-assignment-options clearfix">';
	$key = 'page';
	...
	$output .= '<div id="' . $pre . '-assignment-tab-categories-singles" class="mantra-audience-assignment-options clearfix">';
	$key = 'category_single';
	...
	// Categories tab
	$output .= '<div id="' . $pre . '-assignment-tab-categories" class="mantra-audience-assignment-options clearfix">';
	$key = 'category';
	if ( ! empty( $terms ) ) :
		foreach ( $terms as $term ) :
			$checked = isset( $selected['tax'][$key][$term->slug] ) ? checked( $selected['tax'][$key][$term->slug], 'on', false ) : '';
	...
	// Post types tab
	$output .= '<div id="' . $pre . '-assignment-tab-post-types" class="mantra-audience-assignment-options clearfix">';
	$output .= '<div id="' . $pre . '-mantra-audience-assignment-post-types-inner-tabs" class="mantra-audience-assignment-inner-tabs">';
	...
	// Taxonomies tab
	$output .= '<div id="' . $pre . '-assignment-tab-taxonomies" class="mantra-audience-assignment-options clearfix">';
	$output .= '<div id="' . $pre . '-mantra-audience-assignment-taxonomies-inner-tabs" class="mantra-audience-assignment-inner-tabs">';
	$output .= '<ul class="inline-tabs clearfix">';
	...
	// User Roles tab
	$output .= '<div id="' . $pre . '-assignment-tab-userroles" class="mantra-audience-assignment-options clearfix">';
 ...
 * @return string
 */
function mantra_audience_get_uploader( $id = '', $args = array() ){

	$defaults = array(
		'label'		=> __('Upload', 'mantra-audience'),
		'preset'	=> false,
		'preview'   => false,
		'tomedia'	=> false,
		'topost'	=> '',
		...
		'type'		=> 'image',
		'action'    => 'mantra_audience_plupload',
	);
	...
	if ( is_multisite() && !is_upload_space_available() ) {
		if( '' != $args['message'] ){
			...
			$html = '<small>' . sprintf( __( 'Sorry, you have filled your %s MB storage quota so uploading has been disabled.', 'mantra-audience' ), get_space_allowed() ) . '</small>';
		}
	} else {
		if( '' != $args['fallback'] )
			$html = $args['fallback'];
			...
			<?php } ?>
				...
				<div class="plupload-upload-uic hide-if-no-js <?php echo esc_attr( $classes ); ?>" <?php echo implode( ' ', $datas ); ?> id="<?php echo esc_attr( $id ); ?>plupload-upload-ui">
					...
					<span class="ajaxnonceplu" id="ajaxnonceplu<?php echo wp_create_nonce($id . 'mantra-audience-plupload'); ?>"></span>

				</div>

			<?php if( $args['dragfiles'] ){ ?>
					<small><?php _e(' or drag files here.', 'mantra-audience'); ?></small>
				</div>
			<?php } else { ?>
	...
	if( $args['medialib'] ) {
		...
		<?php
			if ( 'audio' == $args['type'] ) {
				$data_uploader_title = __( 'Browse Audio', 'mantra-audience' );
				$medialib_btn_text = __('Insert Audio', 'mantra-audience');
			} elseif ( 'video' == $args['type'] ) {
				$data_uploader_title = __( 'Browse Video', 'mantra-audience' );
				$medialib_btn_text = __('Insert Video', 'mantra-audience');
			} else {
				$data_uploader_title = __( 'Browse Image', 'mantra-audience' );
				$medialib_btn_text = __('Insert Image', 'mantra-audience');
			}
			$medialib_datas = array(
				'action' => 'mantra_audience_metabox_media_lib_browse',
		...
		?>
		<div class="mantra_audience_medialib_wrapper">
			<?php if($upload_visible): echo '&nbsp;<em>' . __( 'or', 'mantra-audience' ) . '</em>'; endif; ?>&nbsp;
			<a href="#" class="mantra-audience-media-lib-browse" data-submit='<?php echo json_encode( $medialib_datas ); ?>' data-uploader-title="<?php echo esc_attr( $data_uploader_title ) ?>" data-uploader-button-text="<?php echo esc_attr( $medialib_btn_text ); ?>" data-fields="<?php echo esc_attr( $args['fields'] ); ?>" data-type="<?php echo esc_attr( $args['type'] ); ?>"><?php _e( 'Browse Library', 'mantra-audience' ) ?></a>
		</div>
		<?php
		$html .= ob_get_contents();
		ob_end_clean();
 ...
 *
 * @uses mantra_audience_get_uploader
 *
 * @param string $id
 * @param array $args
 *
 */
function mantra_audience_uploader($id = '', $args = array()){
	echo mantra_audience_get_uploader( $id, $args );
}

function mantra_audience_wp_ajax_plupload_image() {
	$imgid = $_POST['imgid'];
	! empty( $_POST[ '_ajax_nonce' ] ) && check_ajax_referer($imgid . 'mantra-audience-plupload');
	...
	$file = wp_handle_upload($_FILES[$imgid . 'async-upload'], array('test_form' => true, 'action' => 'mantra_audience_plupload'));
 ...
 * @todo: remove this
 */
function mantra_audience_metabox_media_lib_browse() {
...
function mantra_audience_meta_field_gallery_shortcode( $args ) {
	extract( $args );
	...
	?>
	<textarea name="<?php echo esc_attr( $meta_box['name'] ); ?>" size="55" rows="4" class="mantra_audience_input_field mantra-audience-gallery-shortcode-input <?php echo esc_attr( $class ); ?>"><?php echo esc_textarea( $meta_value ); ?></textarea>

	<div class="mantra-audience-gallery-shortcode-btn">
		<a href="#" class="mantra-audience-gallery-shortcode-btn"><?php _e('Insert Gallery', 'mantra-audience') ?></a>
	</div>
...
function mantra_audience_meta_field_multi( $args, $call_before_after = true, $echo = true ) {
	global $post;
	extract( $args, EXTR_OVERWRITE );

	foreach ($meta_box['meta']['fields'] as $field) {
		if ( is_callable( 'mantra_audience_meta_field_'.$field['type'] ) ) {
			// @todo
			...
			$call_args = array(
				...
				'post_id' => $post_id,
				'mantra_audience_custom_panel_nonce' => $mantra_audience_custom_panel_nonce,
			);
			...
			if ( isset( $field['enable_toggle'] ) ) {
				...
				echo '<div class="mantra-audience-toggle ' . $field_toggle . '">';
			}

			// Render the field
			call_user_func('mantra_audience_meta_field_'.$field['type'], $call_args);
	...
	if ( isset( $meta_box['meta']['description'] ) && '' != $meta_box['meta']['description'] ) {
		echo '<span class="mantra_audience_field_description">' . $meta_box['meta']['description'] . '</span>';
	}
}

if( ! function_exists( 'mantra_audience_meta_field_query_category' ) ) :
/**
...
function mantra_audience_meta_field_query_category( $args ) {
	...
	if( ! empty( $terms_by_tax ) && ! is_wp_error( $terms_by_tax ) ) {
		$terms_list = array();
		$terms_list['0'] = array(
			'title' => __( 'All Categories', 'mantra-audience' ),
			'slug'	=> '0'
		);
...
function mantra_audience_meta_field_repeater_template( $meta_box, $data, $id ) {
	echo '<div class="mantra-audience-repeater-row" data-id="'. $id .'">';
	echo '<div class="mantra-audience-repeater-remove-row"><a href="#"></a></div>';
	foreach ( $meta_box['fields'] as $field ) {
		if ( is_callable( 'mantra_audience_meta_field_'.$field['type'] ) ) {
			echo '<div class="mantra_audience_field_row clearfix ">';
			! empty( $field['title'] ) && printf( '<div class="mantra_audience_field_title">%s</div>', $field['title'] );
			$field_id = $field['name'];
			...
			$call_args = array(
				...
				'call_before_after' => false,
				'post_id' => 0,
				'mantra_audience_custom_panel_nonce' => 0,
			);
			...
			printf( '<div class="mantra_audience_field mantra_audience_field-%s">', $field['type'] );
				// Render the field
				call_user_func('mantra_audience_meta_field_'.$field['type'], $call_args);
			echo '</div>';
...
function mantra_audience_meta_field_repeater( $args, $call_before_after = true, $echo = true ) {
	global $post;
	extract( $args, EXTR_OVERWRITE );

	echo '<script type="text/html" class="mantra-audience-repeater-template">';
		mantra_audience_meta_field_repeater_template( $meta_box, array(), '__i__' );
	echo '</script><!-- .mantra-audience-repeater-template-->';

	echo '<div class="mantra-audience-repeater-rows">';
		if( isset( $meta_value ) && is_array( $meta_value ) ) {
			foreach( $meta_value as $id => $values ) {
				mantra_audience_meta_field_repeater_template( $meta_box, $values, $id );
			}
		}
	echo '</div><!-- .mantra-audience-repeater-rows -->';

	$label = isset( $meta_box['add_new_label'] ) ? $meta_box['add_new_label'] : __( 'Add New', 'mantra-audience' );
	echo '<button class="mantra-audience-repeater-add button button-secondary">' . $label . '</button>';
