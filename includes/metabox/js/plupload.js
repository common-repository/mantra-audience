function mantra_audience_create_pluploader(obj) {
	...
	uploader.bind('Error', function(up, error){
		...
		if( -600 == error.code ){
			var errorMessage = mantra_audience_lang.filesize_error,
				errorMessageFix = mantra_audience_lang.filesize_error_fix;
		}
	...
	uploader.bind('FileUploaded', function(up, file, response) {
		...
		if(json.error){
			...
			if($j('.prompt-box .show-error').length > 0){
				...
				$j('.prompt-box .show-error').append('<p>' + mantra_audience_lang.enable_zip_upload + '</p>');
			}
			$j(".overlay, .prompt-box").fadeIn(500);
			return;
		}
		...
		if ( typeof json.thumb !== 'undefined' ) {
			mantraAudienceMediaLib.setPreviewIcon($this.closest( '.mantra_audience_field, .mantra_audience_field_row' ), json.thumb );
			$j( 'body' ).trigger( 'mantra_audience_plupload_selected', [ $this, json ] );
		}
