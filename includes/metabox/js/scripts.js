window.Mantra_Audience_Metabox = (function($){
	...
	api.init_metabox_tabs = function(){
		...
		$( '.mantra-audience-meta-box-tabs' ).each(function(){
			var context = $( this );
		...
		$('.ilc-htabs a').on('click', function(){
			api.set_cookie('mantra-audience-builder-tabs', $(this).attr('id'));
		});

		// active tabs cookie
		if( typeof(api.get_cookie('mantra-audience-builder-tabs')) != 'undefined' && api.get_cookie('mantra-audience-builder-tabs') !== null ){
			$( '#' + api.get_cookie('mantra-audience-builder-tabs')).trigger('click');
		}
	}
	...
	api.init_fields = function( $context ) {
		...
		$( document ).trigger( 'mantra_audience_metabox_init_fields', [api] );
	}

	api.repeater = function(){
		$( 'body' ).on( 'click', '.mantra-audience-repeater-add', function(e){
			e.preventDefault();
			var $this = $( this ),
				container = $this.closest( '.mantra_audience_field_row' ),
				rows = container.find( '.mantra-audience-repeater-rows' ),
				template = container.find( '.mantra-audience-repeater-template' ).html();
			...
			if( $template.has( '.plupload-upload-uic' ).length ) {
				$template.find( '.plupload-upload-uic' )
					.each( function () {
						mantra_audience_create_pluploader( $( this ) );
					} );
			}
			// init field types for the new row
			api.init_fields( rows.find('.mantra-audience-repeater-row:last-child') );
		} );

		$( 'body' ).on( 'click', '.mantra-audience-repeater-remove-row', function( e ) {
			e.preventDefault();
	...
	api.audioRemoveAction = function() {
		$( 'body' ).on( 'click', '[data-audio-remove] a', function( e ) {
			...
			callback();
			data.action = 'mantra_audience_remove_audio';
	...
	api.date_picker = function( $context ) {
		$context.find( '.mantraAudienceDatePicker' ).each(function(){
			var $self = $(this),
			...
			( $.fn.mantraAudienceDatetimepicker
				? $.fn.mantraAudienceDatetimepicker
				: $.fn.datetimepicker ).call( $self, {
					...
					onClose: function( date ) {
						if ( '' != date ) {
							$( '#' + $self.data('clear') ).addClass('mantraAudienceFadeIn');
						}
					},
					beforeShow: function() {
						$('#ui-datepicker-div').addClass( 'mantraAudienceDateTimePickerPanel' );
					}
				});
			$self.next().addClass('button');
		});

		$context.find( '.mantraAudienceClearDate' ).on( 'click', function(e) {
			e.preventDefault();
			var $self = $(this);
			$( '#' + $self.data('picker') ).val('').trigger( 'change' );
			$self.removeClass('mantraAudienceFadeIn');
		});
	}

	api.assignments = function( $context ){
		$context.find( '.mantra-audience-assignments, .mantra-audience-assignment-inner-tabs' ).tabs();
	}
	...
	api.layout = function( $context ) {
		$context.find( '.preview-icon' ).each( function() {
			...
			} else if ( $self.closest('.mantra_audience_field_row').length > 0 ) {
				context = 'custom-panel';
				$dataHide = $self.closest('.mantra_audience_field_row');
			...
			$self.click(function(e){
				...
				if ( '' !== dataHide ) {
					if ( 'custom-panel' == context ) {
						...
						$list.add( $list.find( '.mantra_audience_field .hide-if' ) ).not('.toggled-off').filter( '.' + dataHide.replace( /\s/g, ',.' ) ).show().filter( '.' + $val.val() ).hide();
			...
			if ( '' !== dataHide ) {
				if ( 'custom-panel' == context ) {
					var $list = $dataHide.nextUntil('[data-hide]');
					$list.add( $list.find( '.mantra_audience_field .hide-if' ) ).not('.toggled-off').filter( '.' + dataHide.replace( /\s/g, ',.' ) ).filter( '.' + $val.val() ).hide();
				} else if ( 'theme-settings' == context ) {
					$dataHide.find('.hide-if').filter( '.' + dataHide.replace( /\s/g, ',.' ) ).show().filter( '.' + $val.val() ).hide();
				}
			}
		 ...
		 */
		$context.find( ".mantra_audience_field .preview-icon" ).on( 'click', function(e){
			e.preventDefault();
		...
		$context.find( '.mantra_audience_field_row[data-hide]' ).each( function() {
	...
	api.gallery_shortcode = function(){
		...
		$( 'body' ).on( 'click', '.mantra-audience-gallery-shortcode-btn', function(event) {
			var shortcode_val = $(this).closest('.mantra_audience_field').find('.mantra-audience-gallery-shortcode-input');
	...
	api.enable_toggle = function(){
		...
		if($enableToggle.length > 0){
			$enableToggle.each(function(){
				var context = $(this).closest('.mantra_audience_write_panel');
				$('.mantra-audience-toggle', context).hide().addClass('toggled-off');
			});
		}
		$('.enable_toggle .preview-icon').on('click', function(e){
			...
			$(this).closest('.inside').find('.mantra-audience-toggle').hide().addClass('toggled-off');
	...
	api.query_category = function(){
		 ...
		 */
		var $mantraAudienceField = $('.mantra_audience_field'),
			$mantraAudienceInfoLink = $('.mantra-audience-info-link');

		$mantraAudienceField.find('.query_category').blur(function(){
		...
		$mantraAudienceField.find('.query_category_single').change(function() {
			var $self = $(this), value = $self.val();
			$self.parent().find('.query_category, .val').val( value );
			toggleQueryCategoryFields( $self, value );
		}).closest('.mantra_audience_field_row').addClass('query-field-visible');
		$mantraAudienceInfoLink.closest('.mantra_audience_field_row').addClass('query-field-visible');

		$('.query_category_single, .query_category').each(function(){
			var $self = $(this), value = $self.val();
			toggleQueryCategoryFields( $self, value );
		});
		$mantraAudienceInfoLink.closest('.mantra_audience_field_row').removeClass('query-field-hide');

		function toggleQueryCategoryFields( $obj, value ) {
			if ( '' != value ) {
				$obj.closest('.inside').find('.mantra_audience_field_row').removeClass('query-field-hide');
			} else {
				$obj.closest('.inside').find('.mantra_audience_field_row').not( $obj.closest( '.mantra_audience_field_row' ) ).not('.query-field-visible').addClass('query-field-hide');
			}
		}
	}
