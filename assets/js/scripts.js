(function($){
	...
	function open( $el ) {
		...
		var view_count = getCookie( "mantra-audience-" + $el.data( 'object-id' ) );
		...
		setCookie( "mantra-audience-" + $el.data( 'object-id' ), view_count + 1, $el.data( 'cookie-expiration' ) );

		var style = $el.data( 'style' );
		$el.show();
		var classes = 'mantra-audience-showing mantra-audience-style-' + style + ' mantra-audience-showing-' + $el.data( 'object-id' ) + ' ma-popup-position-' + $el.data( 'position' );
		if( style == 'classic' || style == 'fullscreen' ) {

			// do not display the popup if one is showing already
			if( $( 'body' ).hasClass( 'mantra-audience-showing' ) ) {
				return;
			}
			...
			$.magnificPopup.open({
				...
				callbacks : {
					open : function(){
						...
						if( typeof mantraAudienceBuilderModuleJs == 'object' ) {
							mantraAudienceBuilderModuleJs.loadOnAjax();
						}

						// move close button to the top-right corner of the screen
						$( '.mfp-close' ).addClass( 'mantra-audience-close' ).appendTo( $( '.mfp-content' ) );
			...
			$el.append( '<button class="mantra-audience-close">x</button>' );
		}
	...
	function close( $el ) {
		...
		if( style == 'classic' || style == 'fullscreen' ) {
			...
			$el.find( '.mantra-audience-close' ).click();
		}
	}

	$( 'body' ).on( 'click', '.mantra-audience .mantra-audience-close', function(){
		var popup = $( this ).closest( '.mantra-audience' );
	...
	$(function(){
		$( '.mantra-audience' ).each( function(){
			var $this = $( this );

			// manual trigger, open popup when a link calls it
			$( 'body' ).on( 'click', '[href="#mantra-audience-' + $this.data( 'object-id' ) + '"]', function(e){
				e.preventDefault();
				open( $this );
			} );
